class RVote < ApplicationRecord
  belongs_to :voter, polymorphic: true, inverse_of: :r_votes_as_voter
  belongs_to :votable, polymorphic: true, inverse_of: :r_votes_as_votable

  validates_inclusion_of :weight, in: [-1, 1]
  validates :voter_id, uniqueness: { scope: %i[voter_type votable_id votable_type] }

  scope :up, -> { where(weight: 1) }
  scope :down, -> { where(weight: -1) }

  after_save :update_tags_cache
  after_destroy :update_tags_cache

  def voter_document
    RDocument.find_or_create_by(recommendable_id: voter_id, recommendable_type: voter_type)
  end

  def votable_document
    RDocument.find_or_create_by(recommendable_id: votable_id, recommendable_type: votable_type)
  end

  def update_tags_cache
    if destroyed?
      voter_document.decrement(self)
    elsif saved_change_to_id?
      voter_document.increment(self)
    elsif saved_change_to_weight?
      voter_document.invert(self)
    end
  end
end
